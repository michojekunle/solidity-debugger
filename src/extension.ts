...
export function activate(context: vscode.ExtensionContext) {
  console.log("Smart Contract Debugger extension is now active");

  // Register the debug adapter provider
  const provider = new SolidityDebuggerProvider(context);
  context.subscriptions.push(
    vscode.debug.registerDebugAdapterDescriptorFactory("soldbg", provider),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("soldbg.showStateVisualizer", () => {
      StateVisualizerPanel.createOrShow(
        context.extensionUri,
        stateProcessorService,
      );
    }),
    vscode.commands.registerCommand("soldbg.showGasAnalyzer", () => {
      GasAnalyzerPanel.createOrShow(context.extensionUri, gasAnalyzerService);
    }),
    vscode.commands.registerCommand("soldbg.showHelp", () => {
...
export class StateProcessorService implements vscode.Disposable {
  ...
  private registerCommands() {
    ...
    const analyzeDeployedCommand = vscode.commands.registerCommand(
      "soldbg.analyzeDeployedContract",
      async () => {
    ...
    const simulateFunctionCommand = vscode.commands.registerCommand(
      "soldbg.simulateContractFunction",
  ...
  public async analyzeContractState() {
    try {
      ...
      vscode.commands.executeCommand("soldbg.showStateVisualizer");
    } catch (error) {
  ...
  private async analyzeDeployedContract() {
    try {
      const address = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
          const validation =
            this.validationService.validateEthereumAddress(value);
      ...
      vscode.window.withProgress(
        ...
        async () => {
          try {
            const success =
              await this.stateCollector.analyzeDeployedContract(address);
            ...
            if (
              ...
              console.error(
                "[GasTour] Failed to start gas tour due to missing dependency:",
                error,
              );
  ...
  private async analyzeTransaction() {
    try {
      const txHash = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
          const validation =
            this.validationService.validateTransactionHash(value);
          return validation.valid ? null : validation.error;
        },
      });
      ...
      vscode.window.withProgress(
        ...
        async () => {
          try {
            const success =
              await this.stateCollector.analyzeTransaction(txHash);
   ...
   */
  public async simulateContractFunction(
    functionName: string,
    inputs: any[],
    currentState?: Record<string, any>,
  ) {
    try {
      ...
      const result = await this.contractSimulator.simulateFunction(
        functionName,
        inputs,
        state,
      );
...
export class GasAnalyzerService implements vscode.Disposable {
  ...
  private gasDecorationManager: GasDecorationManager =
    new GasDecorationManager();
  ...
  private registerCommands() {
    const analyzeGasCommand = vscode.commands.registerCommand(
      "soldbg.analyzeGasUsage",
      () => {
        this.analyzeGasUsage();
      },
    );

    const startGasTourCommand = vscode.commands.registerCommand(
      "soldbg.startGasTour",
      () => {
        this.startGasTour();
      },
    ...
    const nextHotspotCommand = vscode.commands.registerCommand(
      "soldbg.nextGasHotspot",
      () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
    ...
    const prevHotspotCommand = vscode.commands.registerCommand(
      "soldbg.previousGasHotspot",
      () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
    ...
    const finishTourCommand = vscode.commands.registerCommand(
      "soldbg.finishGasTour",
      () => {
        this.finishGasTour();
      },
    );
  ...
  public async analyzeGasUsage() {
    try {
      ...
      const contractData = Compiler.getContractFromOutput(
        compilationOutput,
        filePath,
      );
      ...
      this.gasEstimator.processTrace(
        contractData.bytecode,
        contractData.contractName,
      );

      const usageData = this.gasEstimator.getGasUsageForFunction(
        contractData.contractName,
      );
      ...
      while ((match = functionPattern.exec(contractCode)) !== null) {
        ...
        if (functionName !== contractData.contractName) {
          results.push({
            ...
            recommendations: [
              "Run contract interaction to get precise gas",
              "Static analysis unavailable for specific function subset",
            ],
          });
        }
      }
      ...
      vscode.window.showInformationMessage(
        `Analyzed contract ${contractData.contractName}`,
      );
    } catch (error) {
    ...
    try {
      ...
      vscode.window.withProgress(
        ...
        async () => {
          ...
          if (!contractData) {
            console.error(
              "[Gas Tour] No contract data returned from compilation",
            );
            throw new Error(
              "Could not extract contract data from compilation output. Check console for details.",
            );
          }
          ...
          const contractName = Object.keys(
            compilationOutput.contracts[fileName],
          )[0];
          ...
          if (!contractData.bytecode) {
            ...
            throw new Error(
              `Bytecode not found for contract ${contractData.contractName}. The contract may have compilation errors.`,
            );
          }

          console.log(
            "[Gas Tour] Bytecode length:",
            contractData.bytecode.length,
          );
          ...
          const blocks = blockAnalyzer.analyzeBlocks(
            hotspots,
            contractCode,
            editor.document,
          );
          ...
          console.log(
            "[Gas Tour] Generated",
            enhancedHotspots.length,
            "enhanced hotspots from",
            blocks.length,
            "blocks",
          );

          // Apply decorations
          this.gasDecorationManager.applyTourDecorations(
            editor,
            enhancedHotspots,
          );
          ...
          this.disposables.push(
            this.gasTourProvider.onStepChanged(
              ({ current, total, hotspot }) => {
                this.gasDecorationManager.highlightCurrentHotspot(
                  editor,
                  hotspot,
                );
                vscode.window.showInformationMessage(
            ...
            this.gasTourProvider.onTourEnded(() => {
              ...
              this.gasDecorationManager.convertToDiagnostics(
                editor.document,
                hotspots,
              );
...
export class EducationalContentService implements vscode.Disposable {
  ...
  private registerCommands() {
    const showTutorialCommand = vscode.commands.registerCommand(
      "soldbg.showSmartContractTutorial",
      () => {
        this.showTutorial();
      },
    );
