...
export class StateCollector implements vscode.Disposable {
  ...
  public async analyzeActiveContract() {
    try {
      ...
      const compilationOutput = await Compiler.compile(content, filePath);
      const contractData = Compiler.getContractFromOutput(compilationOutput, filePath);

      if (!contractData) {
        throw new Error("Compilation returned no result or contract not found");
      }
  ...
  private processContractData(data: { contractName: string; abi: any[]; storageLayout: any }) {
    this.currentContractName = data.contractName;
    this.currentContractAbi = data.abi || [];
    this.storageLayout = data.storageLayout;
  ...
  public async traceCall(transaction: any): Promise<any> {
    try {
         if (!this.provider) {
             throw new Error("No provider available for tracking call");
         }
         
         // Use debug_traceCall if available
         const trace = await this.provider.send("debug_traceCall", [transaction, "latest", { tracer: "callTracer" }]);
         return trace;
    } catch (error) {
        // Fallback or rethrow
        throw error;
  ...
  public getCurrentContractAddress(): string | null {
